...
describe('Plane Game 项目测试', () => {
    beforeEach(() => {
      ...
      cy.viewport(800, 800);
      // 如果需要复现措测试，根据自己的路径修改
      cy.visit('/March13-src/index.html');
  
      // 固定部分 DOM 样式和全局变量，确保 getStyle() 返回固定值（然而还是没有用，这个环境还是得不到动态计算值）
      cy.window().then((win) => {
        ...
        // 移除对 SHIELD_DURATION/backBtn 的 export 依赖, 改为仅 window 访问
        if (typeof win.SHIELD_DURATION === 'undefined') {
