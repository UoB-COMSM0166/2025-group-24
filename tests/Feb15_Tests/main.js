...
function moveBullet(b) {
  ...
  /**
 * 暂停所有玩家子弹的移动
 * 通过清除 `setInterval` 使子弹停止移动
 */
function pauseAllBullets() {
  for (var i = 0; i < bullets.length; i++) {
      clearInterval(bullets[i].timer); // 停止当前子弹的移动定时器
...
function resumeAllBullets() {
  for (var i = 0; i < bullets.length; i++) {
      if (!bullets[i].timer) { // 确保不会重复启动 `setInterval`
...
function moveEnemy(e) {
  ...
  e.timer = setInterval(function () {
      ...
      // 如果敌机超出游戏区域，移除它
      if (topVal >= gameH) {
          clearInterval(e.timer); // 清除当前敌机的移动定时器
          if (e.parentNode) {
...
/**
* 暂停所有敌机
* 清除所有敌机的 `setInterval` 定时器，使其停止移动
*/
function pauseAllEnemies() {
  clearInterval(enemyTimer); // 停止新的敌机生成
